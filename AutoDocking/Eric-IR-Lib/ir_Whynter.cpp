...
//+=============================================================================
#if SEND_WHYNTER
void  IRsend::sendWhynter (unsigned long data,  int nbits, int pin)
{
	// Set IR carrier frequency
	enableIROut(38, pin);

	// Start
	mark(WHYNTER_ZERO_MARK, pin);
	space(WHYNTER_ZERO_SPACE, pin);

	// Header
	mark(WHYNTER_HDR_MARK, pin);
	space(WHYNTER_HDR_SPACE, pin);

	// Data
	for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
			mark(WHYNTER_ONE_MARK, pin);
			space(WHYNTER_ONE_SPACE, pin);
		} else {
			mark(WHYNTER_ZERO_MARK, pin);
			space(WHYNTER_ZERO_SPACE, pin);
		}
	...
	// Footer
	mark(WHYNTER_ZERO_MARK, pin);
	space(WHYNTER_ZERO_SPACE, pin);  // Always end with the LED off
}
#endif
...
bool  IRrecv::decodeWhynter (decode_results *results)
{
	...
	if (recvparams.rawlen < (2 * WHYNTER_BITS) + 6)  return false ;
