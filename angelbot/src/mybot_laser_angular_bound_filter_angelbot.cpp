...
bool my_update(const sensor_msgs::LaserScan& input_scan)
{
  ...
  for (unsigned int i = 0; i < input_scan.ranges.size(); ++i){
   ...
   if(start_angle >= input_scan.angle_min && start_angle < lower_angle_) {
    ...
    else if (start_angle > lower_angle_ && start_angle < upper_angle_){
        filtered_scan.ranges[count] = +INFINITY;
        current_angle += input_scan.angle_increment;
        start_angle += input_scan.angle_increment;
        count++;
	}
  ...
  filtered_scan.angle_min = input_scan.angle_min;//start_angle;
...
int main(int argc, char** argv){
  ros::init(argc, argv, "mybot_laser_angular_bound_filter_andbot1dot2");
  ros::NodeHandle n1, n2;
  input_scan_sub = n1.subscribe("/scan", 10, input_scan_sub_callback);
  filtered_scan_pub = n2.advertise<sensor_msgs::LaserScan>("/rplidar_scan", 50);
