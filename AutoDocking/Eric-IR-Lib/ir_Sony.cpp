...
//+=============================================================================
#if SEND_SONY
void  IRsend::sendSony (unsigned long data,  int nbits, int pin)
{
	// Set IR carrier frequency
	enableIROut(40, pin);

	// Header
	mark(SONY_HDR_MARK, pin);
	space(SONY_HDR_SPACE, pin);

	// Data
	for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
			mark(SONY_ONE_MARK, pin);
			space(SONY_HDR_SPACE, pin);
		} else {
			mark(SONY_ZERO_MARK, pin);
			space(SONY_HDR_SPACE, pin);
    	}
  	}
...
bool  IRrecv::decodeSony (decode_results *results)
{
	...
	if (recvparams.rawlen < (2 * SONY_BITS) + 2)  return false ;
...
#	endif
	...
	while (offset + 1 < recvparams.rawlen) {
