...
//+=============================================================================
#if SEND_SAMSUNG
void  IRsend::sendSAMSUNG (unsigned long data,  int nbits, int pin)
{
	// Set IR carrier frequency
	enableIROut(38, pin);

	// Header
	mark(SAMSUNG_HDR_MARK, pin);
	space(SAMSUNG_HDR_SPACE, pin);

	// Data
	for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
			mark(SAMSUNG_BIT_MARK, pin);
			space(SAMSUNG_ONE_SPACE, pin);
		} else {
			mark(SAMSUNG_BIT_MARK, pin);
			space(SAMSUNG_ZERO_SPACE, pin);
		}
	}
...
bool  IRrecv::decodeSAMSUNG (decode_results *results)
{
	...
	if (recvparams.rawlen < (2 * SAMSUNG_BITS) + 4)  return false ;
