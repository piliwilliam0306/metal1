...
//
#if SEND_JVC
void  IRsend::sendJVC (unsigned long data,  int nbits,  bool repeat, int pin)
{
	...
	if (!repeat){
		mark(JVC_HDR_MARK, pin);
		space(JVC_HDR_SPACE, pin);
	}

	// Data
	for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
			mark(JVC_BIT_MARK, pin);
			space(JVC_ONE_SPACE, pin);
		} else {
			mark(JVC_BIT_MARK, pin);
			space(JVC_ZERO_SPACE, pin);
		}
	}
...
bool  IRrecv::decodeJVC (decode_results *results)
{
	...
	// Check for repeat
	if (  (recvparams.rawlen - 1 == 33)
	    && MATCH_MARK(results->rawbuf[offset], JVC_BIT_MARK)
	    && MATCH_MARK(results->rawbuf[recvparams.rawlen-1], JVC_BIT_MARK)
	   ) {
		results->bits        = 0;
	...
	if (recvparams.rawlen < (2 * JVC_BITS) + 1 )  return false ;
