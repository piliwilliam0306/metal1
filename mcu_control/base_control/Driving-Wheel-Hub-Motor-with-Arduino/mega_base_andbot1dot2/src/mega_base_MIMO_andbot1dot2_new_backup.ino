...
double fb_omega_left = 0;
double vol_target_right = 0;
double fb_omega_right = 0;
...
void getMotorData_left() {
  ...
  fb_omega_left = ((EncodeDiff_left) * (1000 / dT)) * 2 * PI / (CPR);
...
void getMotorData_right() {
  ...
  fb_omega_right = ((EncodeDiff_right) * (1000 / dT)) * 2 * PI / (CPR);
...
void Feedback_wheel_angularVel()
{
  //limit 1 rev/sec
  //if(WHEEL_SELECT==0) //left wheel
    fb_omega_left= int(-1 * fb_omega_left / (MAX_AngularSpeed /double(32767)));           //convert received 16 bit integer to actual speed 6.283/32767=1.917477950376904e-4=0.0001917477950376904
  //else if(WHEEL_SELECT==1) //right wheel
    fb_omega_right = int(fb_omega_right / (MAX_AngularSpeed /double(32767)));           //convert received 16 bit integer to actual speed 6.283/32767=1.917477950376904e-4=0.0001917477950376904
...
void loop()
{
    ...
    if ((millis() - lastMilli) >= LOOPTIME)
    {
        ...
        getMotorData();
        Feedback_wheel_angularVel();    //send actually speed to mega
        if (vol_target == 0)
                        {
                                Vq = 0;
                                //sum_error = 0;
