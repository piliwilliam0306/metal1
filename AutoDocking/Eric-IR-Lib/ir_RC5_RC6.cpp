...
//+=============================================================================
#if SEND_RC5
void  IRsend::sendRC5 (unsigned long data,  int nbits, int pin)
{
	...
	for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
		if (data & mask) {
			space(RC5_T1, pin); // 1 is space, then mark
			mark(RC5_T1, pin);
		} else {
			mark(RC5_T1, pin);
			space(RC5_T1, pin);
...
bool  IRrecv::decodeRC5 (decode_results *results)
{
	...
	if (recvparams.rawlen < MIN_RC5_SAMPLES + 2)  return false ;
	...
	for (nbits = 0;  offset < recvparams.rawlen;  nbits++) {
...
#if SEND_RC6
void  IRsend::sendRC6 (unsigned long data,  int nbits, int pin)
{
	// Set IR carrier frequency
	enableIROut(36, pin);

	// Header
	mark(RC6_HDR_MARK, pin);
	space(RC6_HDR_SPACE, pin);
