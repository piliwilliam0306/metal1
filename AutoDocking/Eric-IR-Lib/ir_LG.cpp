...
bool  IRrecv::decodeLG (decode_results *results)
{
    ...
    if (recvparams.rawlen < (2 * LG_BITS) + 1 )  return false ;
...
//+=============================================================================
#if SEND_LG
void  IRsend::sendLG (unsigned long data,  int nbits, int pin)
{
    // Set IR carrier frequency
    enableIROut(38, pin);

    // Header
    mark(LG_HDR_MARK, pin);
    space(LG_HDR_SPACE, pin);
    mark(LG_BIT_MARK, pin);

    // Data
    for (unsigned long  mask = 1UL << (nbits - 1);  mask;  mask >>= 1) {
        if (data & mask) {
            space(LG_ONE_SPACE, pin);
            mark(LG_BIT_MARK, pin);
        } else {
            space(LG_ZERO_SPACE, pin);
            mark(LG_BIT_MARK, pin);
        }
    }
