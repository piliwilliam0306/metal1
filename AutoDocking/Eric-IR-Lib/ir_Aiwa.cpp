...
void  IRsend::sendAiwaRCT501 (int code, int pin)
{
	unsigned long  pre = 0x0227EEC0;  // 26-bits

	// Set IR carrier frequency
	enableIROut(AIWA_RC_T501_HZ, pin);

	// Header
	mark(AIWA_RC_T501_HDR_MARK, pin);
	space(AIWA_RC_T501_HDR_SPACE, pin);

	// Send "pre" data
	for (unsigned long  mask = 1UL << (26 - 1);  mask;  mask >>= 1) {
		mark(AIWA_RC_T501_BIT_MARK, pin);
		if (pre & mask)  space(AIWA_RC_T501_ONE_SPACE, pin) ;
		else             space(AIWA_RC_T501_ZERO_SPACE, pin) ;
	}
...
//    I suspect TOPBIT should be changed to 0x00008000
	...
	for (int  i = 0;  i < 15;  i++) {
		mark(AIWA_RC_T501_BIT_MARK, pin);
		if (code & 0x80000000)  space(AIWA_RC_T501_ONE_SPACE, pin) ;
		else                    space(AIWA_RC_T501_ZERO_SPACE, pin) ;
		code <<= 1;
	}
...
//-^- THIS CODE LOOKS LIKE IT MIGHT BE WRONG - CHECK!
	...
	// POST-DATA, 1 bit, 0x0
	mark(AIWA_RC_T501_BIT_MARK, pin);
	space(AIWA_RC_T501_ZERO_SPACE, pin);
...
bool  IRrecv::decodeAiwaRCT501 (decode_results *results)
{
	...
	// Check SIZE
	if (recvparams.rawlen < 2 * (AIWA_RC_T501_SUM_BITS) + 4)  return false ;
	...
	while(offset < recvparams.rawlen - 4) {
