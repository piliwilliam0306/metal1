  ...
  angular_0(0),angular_1(0),angular_increment_0(0.005),angular_increment_1(0.005),speed(s),mode(m)
{
  ...
  offset_pub_0 = nh_.advertise<andbot::JointOffset>("andbot/joint/L/cmd/offset_v", 1);
  offset_pub_1 = nh_.advertise<andbot::JointOffset>("andbot/joint/R/cmd/offset_v", 1);
...
void TeleopHome::keyLoop()
{
  ...
  for(;;)
  {
    ...
    andbot::JointOffset offset_L,offset_R;   
    if(mode==0)
	{
		offset_L.joint0.offset = angular_0;
    		offset_L.joint1.offset = angular_1;
		offset_pub_0.publish(offset_L);
	}
    if(mode==1)
	{
    		offset_L.joint3.offset = angular_0;
    		offset_L.joint2.offset = angular_1;
		offset_pub_0.publish(offset_L);
	}
    if(mode==2)
	{
	    	offset_R.joint0.offset = angular_0;
	    	offset_R.joint1.offset = angular_1;
		offset_pub_1.publish(offset_R);
	}
    if(mode==3)
	{
	    	offset_R.joint3.offset = angular_0;
	    	offset_R.joint2.offset = angular_1;
		offset_pub_1.publish(offset_R);
	}
